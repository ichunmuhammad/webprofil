...
(function() {
	...
	(function() {
		...
		CKEDITOR.tools = {
			...
			extend: function(a) {
				...
				for (var g = 1; g < b; g++) {
					var m = arguments[g] || {};
					CKEDITOR.tools.array.forEach(CKEDITOR.tools.object.keys(m), function(
						b
					) {
	...
	(function() {
		...
		CKEDITOR.tools.extend(CKEDITOR.editor.prototype, {
			...
			destroy: function(a) {
				...
				CKEDITOR.tools.array.forEach(CKEDITOR.tools.object.keys(b), function(
					a
				) {
					a = b[a];
	...
	(function() {
		...
		CKEDITOR.template.prototype.output = function(d, b) {
			var c = ("function" === typeof this.source
				? this.source(d)
				: this.source
		...
		CKEDITOR.editable = CKEDITOR.tools.createClass({
			...
			proto: {
				...
				getUniqueId: function() {
					var a;
					try {
						this._.expandoNumber = a = CKEDITOR.dom.domObject.prototype.getUniqueId.call(
							this
						);
					} catch (b) {
				...
				if (CKEDITOR.env.ie || CKEDITOR.env.gecko || A)
					m
						...
						g.attachListener(
							...
							function() {
								var a = p && (p.isFake || 2 > p.getRanges());
...
*/
	...
	(function() {
		...
		function W(a) {
			var b = CKEDITOR.dom.element.createFromHtml(
					CKEDITOR.addTemplate("dialog", X).output({
		...
		function aa(a) {
			function b(b) {
				var c = "rtl" == f.lang.dir,
					q = k.width,
					l = k.height,
					w = q + (b.data.$.screenX - n.x) * (c ? -1 : 1) * (a._.moved ? 1 : 2),
					B = l + (b.data.$.screenY - n.y) * (a._.moved ? 1 : 2),
					E = a._.element.getFirst(),
					E = c && parseInt(E.getComputedStyle("right")),
					v = a.getPosition();
				v.x = v.x || 0;
				v.y = v.y || 0;
				v.y + B > p.height && (B = p.height - v.y);
				...
				if (
					...
					l = Math.max(e.minHeight || 0, B - h);
				a.resize(q, l);
				a._.moved && N(a, a._.position.x, a._.position.y);
				a._.moved || a.layout();
				b.data.preventDefault();
			}
			function c() {
		...
		CKEDITOR.dialog = function(a, b) {
			...
			function e(a) {
				...
				if (!(1 > b.length)) {
					...
					do {
						e += a;
						if (g && !m._.tabBarMode && (e == b.length || -1 == e)) {
							m._.tabBarMode = !0;
			...
			if (a._.removeDialogTabs && (k = a._.removeDialogTabs[b]))
				for (n = 0; n < k.length; n++) g.removeContents(k[n]);
				...
				function(a) {
					t(function(b) {
						if (b.validate) {
							...
							U.call(b, !d, "string" == typeof c ? c : void 0);
							return d;
						}
					});
				},
			...
			this.on("show", function() {
				A.on("keydown", d, this);
				if (CKEDITOR.env.gecko) A.on("keypress", f, this);
			});
			this.on("hide", function() {
				A.removeListener("keydown", d);
				CKEDITOR.env.gecko && A.removeListener("keypress", f);
				t(function(a) {
					V.apply(a);
				});
			});
			...
			for (n = 0; n < g.contents.length; n++)
				(k = g.contents[n]) && this.addPage(k);
			this.parts.tabs.on(
				"click",
				function(a) {
					var b = a.data.getTarget();
			...
			for (n = 0; n < k.length; n++) this._.buttons[k[n].id] = k[n];
		};
		CKEDITOR.dialog.prototype = {
			...
			hide: function() {
				if (this.parts.dialog.isVisible()) {
					...
					for (ca(this); CKEDITOR.dialog._.currentTop != this; )
			...
			addPage: function(a) {
				if (
					...
					a.accessKey &&
						(T(this, this, "CTRL+" + a.accessKey, da, ea),
			...
			hidePage: function(a) {
				...
				b &&
					1 != this._.pageCount &&
					b.isVisible() &&
					(a == this._.currentTabId && this.selectPage(z.call(this)),
					b.hide(),
					this._.pageCount--,
					this.updateStyle());
			},
		...
		var C = {},
			...
			ca = function(a) {
				for (var b in y) {
					for (var c = y[b], e = c.length - 1; 0 <= e; e--)
				...
				uiElement: function(a, b, c, e, d, f, g) {
					if (!(4 > arguments.length)) {
						...
						b.requiredContent &&
							...
							((n.display = "none"), (this.notAllowed = !0));
						k.id = l;
						var r = {};
						...
						b.align &&
							((m = b.align),
							(n["margin-left"] = "left" == m ? 0 : "auto"),
							(n["margin-right"] = "right" == m ? 0 : "auto"));
						for (l in n) r.push(l + ":" + n[l]);
						...
						CKEDITOR.ui.dialog.uiElement.call(
							...
							function() {
								...
								for (p = 0; p < c.length; p++) {
									...
									g
										? g[p] && e.push("width:" + x(g[p]))
										: e.push("width:" + Math.floor(100 / c.length) + "%");
									h && e.push("height:" + x(h));
									d &&
										void 0 !== d.padding &&
										e.push("padding:" + x(d.padding));
									CKEDITOR.env.ie &&
										CKEDITOR.env.quirks &&
										f[p].align &&
			...
			var a = {
				build: function(a, c, e) {
					for (
						...
						var n = [];
						g.push(n);
						h.push(CKEDITOR.dialog._.uiElementBuilders[f.type].build(a, f, n));
					}
		...
		CKEDITOR.tools.extend(CKEDITOR.editor.prototype, {
			openDialog: function(a, b, c) {
				...
				null === CKEDITOR.dialog._.currentTop && O(this);
	...
	CKEDITOR.plugins.add("dialog", {
		...
		init: function(z) {
			...
			z.on(
				"doubleclick",
				function(D) {
					D.data.dialog && z.openDialog(D.data.dialog);
				},
				null,
				null,
				999
	...
	(function() {
		...
		function w(a) {
			...
			function k(b) {
				...
				d && p.canClipboardApiBeTrusted(c.dataTransfer, a)
					? (b.data.preventDefault(),
					  setTimeout(function() {
							r(a, c);
					  }, 0))
			...
			init: function(a) {
					...
					a.on(
						...
						function(b) {
							...
							if (
								...
								1 == f.getFilesCount() &&
								g != f.id &&
								((f = f.getFile(0)), -1 != CKEDITOR.tools.indexOf(d, f.type))
							) {
				...
				a.on(
					...
					function(b) {
						...
						else if ((n && a.pasteFilter && !b.dontFilter) || k)
							f = v(a, f, a.pasteFilter);
		...
		CKEDITOR.plugins.clipboard = {
			...
			fixSplitNodesAfterDrop: function(a, b, c, d) {
				function g(a, c, d) {
					...
					if (g.equals(c) && d != c.getChildCount())
						return (
				...
				"number" == typeof d &&
					...
					(g(a.startContainer, h, c) || g(a.endContainer, h, d));
			},
			...
			internalDrop: function(a, b, c, d) {
				...
				(f = this.isDropRangeAffectedByDragRange(a, b)) ||
				...
				f = g.startNode;
				b &&
					a.getPosition(f) & CKEDITOR.POSITION_PRECEDING &&
					b.getPosition(f) & CKEDITOR.POSITION_FOLLOWING &&
					f.insertBefore(a);
				a = d.createRange();
				a.moveToBookmark(e);
				h.extractHtmlFromRange(a, 1);
				b = d.createRange();
				g.startNode.getCommonAncestor(h) ||
	...
	(function() {
		...
		CKEDITOR.menu = CKEDITOR.tools.createClass({
			$: function(a, b) {
				...
				var e = CKEDITOR.tools.extend({}, b.panel, {
						css: [CKEDITOR.skin.getPath("editor")],
						level: this._.level - 1,
						block: {}
					}),
					c = (e.block.attributes = e.attributes || {});
				!c.role && (c.role = "menu");
			...
			_: {
				onShow: function() {
					...
					for (var h = 0; h < c.length; h++) {
						...
						if (l)
							for (var f in l) {
								var g = this.editor.getMenuItem(f);
								!g ||
									(g.command && !this.editor.getCommand(g.command).state) ||
									((g.state = l[f]), this.add(g));
							}
					}
				},
				onClick: function(a) {
			...
			proto: {
				...
				show: function(a, b, e, c) {
					...
					if (!f) {
						...
						g = f.addBlock(this.id, this._.panelDefinition.block);
						g.autoSize = !0;
						var d = g.keys;
						d[40] = "next";
						d[9] = "next";
					...
					CKEDITOR.ui.fire("ready", this);
					this.parent
						? this.parent._.panel.showAsChild(f, this.id, a, b, e, c)
						: f.showBlock(this.id, a, b, e, c);
					l.fire("menuShow", [f]);
				},
		...
		CKEDITOR.menuItem = CKEDITOR.tools.createClass({
			...
			proto: {
				render: function(a, b, e) {
					...
					this.command &&
						((g = f.getCommand(this.command)),
						(g = f.getCommandKeystroke(g)) &&
							(d = CKEDITOR.tools.keystrokeToString(
								f.lang.common.keyboard,
								g
							)));
					a = {
						id: k,
						...
						ariaShortcut: d
							? f.lang.common.keyboardShortcut + " " + d.aria
							: "",
	...
	(function() {
		...
		var c =
			...
			n = CKEDITOR.addTemplate("combo", c);
		...
		CKEDITOR.plugins.add("pastefromword", {
			...
			init: function(a) {
											...
											b.type
												? "data:" +
														...
														CKEDITOR.tools.convertBytesToBase64(
															CKEDITOR.tools.convertHexStringToBytes(b.hex)
														)
												: null
										);
									},
				...
				var e = 0,
					...
					f = [
						CKEDITOR.getUrl(f + "filter/common.js"),
				...
				a.pasteTools.register({
					...
					canHandle: function(a) {
						a = a.data.dataValue;
						var b = /(class=\"?Mso|style=(?:\"|\')[^\"]*?\bmso\-|w:WordDocument|<o:\w+>|<\/font>)/,
							b =
					...
					handle: function(f, b) {
						var c = f.data,
							...
							d = { dataValue: d, dataTransfer: { "text/rtf": g } };
						if (!1 !== a.fire("pasteFromWord", d) || e) {
							...
							if (
								...
								(d.dataValue = CKEDITOR.cleanWord(d.dataValue, a)),
									a.fire("afterPasteFromWord", d),
									(c.dataValue = d.dataValue),
									!0 === a.config.forcePasteAsPlainText
										? (c.type = "text")
				...
				if (CKEDITOR.plugins.clipboard.isCustomDataTypesSupported && k)
					a.on("afterPasteFromWord", g);
			}
		});
	})();
			...
			removeformat: {
				exec: function(a) {
					for (
						...
						if (l)
							for (
								m(l), b = b.getNextSourceNode(!0, CKEDITOR.NODE_ELEMENT);
								b && !b.equals(l);
		...
		parseConfig: function(a) {
			...
			"string" !== typeof a.config.scayt_srcUrl &&
				...
				(a.config.scayt_srcUrl =
					c + "//svc.webspellchecker.net/spellcheck31/wscbundle/wscbundle.js"));
			...
			a.config.scayt_disableCache &&
				"boolean" !== typeof a.config.scayt_disableCache &&
				(a.config.scayt_disableCache = !1);
			if (
				void 0 === a.config.scayt_cacheSize ||
				"number" != typeof a.config.scayt_cacheSize ||
				1 > a.config.scayt_cacheSize
			)
				a.config.scayt_cacheSize = 4e3;
		},
		addRule: function(a) {
			var c = CKEDITOR.plugins.scayt,
				d = a.dataProcessor,
		...
		CKEDITOR.plugins.add("stylescombo", {
			...
			init: function(c) {
				...
				c.on("stylesSet", function(a) {
					if ((a = a.data.styles)) {
						for (var b, h, d, e = 0, n = a.length; e < n; e++)
							((b = a[e]),
	...
	(function() {
		function q(d, f) {
			...
			function c(a) {
				0 < e.length ||
					...
					(CKEDITOR.dom.element.setMarker(g, a, "selected_cell", !0),
					e.push(a));
			}
			var e = [],
				g = {};
		...
		function B(d) {
			if (d instanceof CKEDITOR.dom.selection) {
				...
				g = new CKEDITOR.dom.element(
					e[a + 1] || (0 < g ? e[g - 1] : null) || c.$.parentNode
				);
		...
		function u(d, f) {
			for (
				...
				c.push(a[h][g]), e.push(f ? a[h][g - 1] : a[h][g + 1]);
			for (h = 0; h < k; h++)
				c[h] &&
					(1 < c[h].colSpan && e[h] == c[h]
						? ((a = c[h]), (a.colSpan += 1))
						: ((g = new CKEDITOR.dom.element(c[h])),
						  (a = g.clone()),
						  a.removeAttribute("colSpan"),
						  a.appendBogus(),
						  a[f ? "insertBefore" : "insertAfter"].call(a, g),
						  b.push(a),
						  (a = a.$)),
					(h += a.rowSpan - 1));
			return b;
		...
		function y(d) {
			...
			for (d = a.length; m < d; m++)
				for (var n = 0, p = a[m].length; n < p; n++)
					void 0 === k && a[m][n] == e.$ && (k = n), a[m][n] == g.$ && (h = n);
			for (m = k; m <= h; m++)
				for (n = 0; n < a.length; n++)
					(g = a[n]),
						(e = new CKEDITOR.dom.element(c.$.rows[n])),
						(g = new CKEDITOR.dom.element(g[m])),
						g.$ &&
			...
			k =
				a[0].length - 1 > h
					? new CKEDITOR.dom.element(a[0][h + 1])
			...
			if (d instanceof CKEDITOR.dom.selection) {
				...
				a: {
					...
					(g = g.getPrevious())
						? (e = g.getLast())
						: ((g = b[e].getParent()),
						  (e = (g = g.getNext()) ? g.getChild(0) : null));
				}
				d.reset();
		...
		function x(d, f, b) {
			...
			if (f) {
				...
				try {
					...
					n =
						a["up" == f ? l - p : "down" == f ? l + p : l][
							"left" == f ? m - g : "right" == f ? m + g : m
						];
				} catch (x) {
					return !1;
				}
	...
	(function() {
		...
		function z(a, b) {
			...
			for (c = f; c <= k; c++)
				for (f = m; f <= l; f++)
					(d = new CKEDITOR.dom.element(e[c][f])),
						d.$ &&
							!d.getCustomData("selected_cell") &&
							(h.push(d),
							CKEDITOR.dom.element.setMarker(g, d, "selected_cell", !0));
			CKEDITOR.dom.element.clearAllMarkers(g);
			return h;
		}
		function I(a) {
		...
		function L(a, b, c) {
			...
			if (e) (n.first = b || d[0]), (n.dirty = b ? !1 : 1 !== d.length);
				...
				n.active &&
				b &&
				n.first.getAscendant("table").equals(b.getAscendant("table"))
			) {
				d = z(n.first, b);
				if (!n.dirty && 1 === d.length && !D(c.data.getTarget()))
		...
		function w(a) {
			...
			if (
				...
				var e = a.editor || a.listenerData.editor,
					...
					k = J(e),
					h = a.data.getTarget(),
					g = h && h.getAscendant({ td: 1, th: 1 }, !0),
					h = h && h.getAscendant("table", !0),
					m = { table: 1, thead: 1, tbody: 1, tfoot: 1, tr: 1, td: 1, th: 1 };
		...
		function N(a) {
			...
			(b && b.hasAttribute("data-cke-tableselection-ignored")) ||
				((b = a.data.getTarget().getAscendant({ td: 1, th: 1 }, !0)) &&
		...
		function O(a, b) {
			...
			f.getById("cke_table_copybin") ||
				...
				x.append(l),
				a.editable().append(x),
				(n = a.on("selectionChange", c, null, null, 0)),
				g && (p = h.scrollTop),
		...
		q.prototype.insertRow = function(a, b, c) {
			...
			for (
				...
				(k = F(c ? this.cells.all : f, b)),
					(k = CKEDITOR.tools.array.filter(k.find("td, th").toArray(), function(
						a
					) {
		...
		q.prototype.insertColumn = function(a) {
			...
			for (
				var c = this.cells, d = c.all, e = r(c.first), f = r(c.last), c = 0;
				c < a;
				c++
			)
				d = d.concat(CKEDITOR.tools.array.filter(G(d), b));
		...
		var H = {
			onPaste: function(a) {
				...
				var d = a.editor,
					e = d.getSelection(),
					f = e.getRanges(),
					f = f.length && f[0]._getTableElement({ table: 1 });
				if (!f || !f.hasAttribute("data-cke-tableselection-ignored")) {
					...
					!k.length ||
						...
						f
							? (a.stop(),
							  h
									? (g.insertRow(1, 1 === h, !0), e.selectElement(g.rows.first))
									: (g.emptyCells(), t(d, g.cells.all)),
							  (a = g.getTableMap()),
							  (m = CKEDITOR.tools.buildTableMap(f)),
							  g.insertRow(m.length - a.length),
							  g.insertColumn(b(m) - b(a)),
							  (a = g.getTableMap()),
							  this.pasteTable(g, a, m),
							  d.fire("saveSnapshot"),
							  setTimeout(function() {
									d.fire("afterPaste");
							  }, 0))
			...
			pasteTable: function(a, b, c) {
				...
				for (m = 0; m < c.length; m++)
					for (
						h = new CKEDITOR.dom.element(f.$.rows[a.rows.first.$.rowIndex + m]),
							l = 0;
						l < c[m].length;
						l++
					)
						if (
							((g = new CKEDITOR.dom.element(c[m][l])),
							(d = b[m] && b[m][l] ? new CKEDITOR.dom.element(b[m][l]) : null),
							g && !g.getCustomData("processed"))
						) {
							if (d && d.getParent()) g.replace(d);
							else if (0 === l || c[m][l - 1])
								(d = 0 !== l ? new CKEDITOR.dom.element(c[m][l - 1]) : null) &&
								h.equals(d.getParent())
									? g.insertAfter(d)
									: 0 < e
									? h.$.cells[e]
										? g.insertAfter(new CKEDITOR.dom.element(h.$.cells[e]))
										: h.append(g)
									: h.append(g, !0);
							CKEDITOR.dom.element.setMarker(k, g, "processed", !0);
				...
				c.attachListener(
					...
					(function(a) {
						function c(b, e) {
							...
							if (p && p.is && p.is(k)) {
								for (
									var q = p.getAscendant("table", !0),
										u = p.getPreviousSourceNode(!1, CKEDITOR.NODE_ELEMENT, q),
										r = !1,
										t = function(a) {
								...
								!u &&
									...
									!n.is("table") &&
									n.getNext() &&
									((u = n.getNext().findOne("td, th")), (r = !0));
								if (u) f["moveToElementEdit" + (r ? "Start" : "End")](u);
								else
									f.setStartBefore(p.getAscendant("table", !0)), f.collapse(!0);
								l[0].deleteContents();
								return [f];
							}
							if (p) return f.moveToElementEditablePosition(p), [f];
						}
						var f = { 37: 1, 38: 1, 39: 1, 40: 1, 8: 1, 46: 1, 13: 1 },
						...
						return function(h) {
							...
							if (
								...
								r = p[p.length - 1]._getTableElement();
								if (13 !== g || a.plugins.enterkey)
									h.data.preventDefault(), h.cancel();
								if (36 < g && 41 > g)
									p[0].moveToElementEditablePosition(n ? q : r, !n),
										l.selectRanges([p[0]]);
								else if (13 !== g || 13 === k || k === CKEDITOR.SHIFT + 13) {
									for (h = 0; h < p.length; h++) b(p[h]);
									(h = c(q, p))
										? (p = h)
										: p[0].moveToElementEditablePosition(q);
									l.selectRanges(p);
									13 === g && a.plugins.enterkey
										? (a.fire("lockSnapshot"),
										  13 === k
												? a.execCommand("enter")
				...
				c.attachListener(
					...
					function(c) {
						...
						if (
							...
							c = e.getRanges();
							f = c[0].getEnclosedNode().getAscendant({ td: 1, th: 1 }, !0);
							for (k = 0; k < c.length; k++) b(c[k]);
							f &&
								(c[0].moveToElementEditablePosition(f), e.selectRanges([c[0]]));
						}
					},
					null,
					null,
			...
			init: function(a) {
				this.isSupportedEnvironment() &&
					...
					a.on("contentDom", function() {
						...
						CKEDITOR.plugins.clipboard &&
							!CKEDITOR.plugins.clipboard.isCustomCopyCutSupported &&
							(b.attachListener(b, "cut", E), b.attachListener(b, "copy", E));
					}),
					a.on("paste", H.onPaste, H),
					B(
						a,
					...
					B(a, ["cellMerge", "cellMergeRight", "cellMergeDown"], function(
						a,
						c
					) {
		...
		CKEDITOR.tools.extend(CKEDITOR.fileTools, {
						...
						function(d) {
							...
							if (!d.dataValue && f)
								for (n = 0; n < f; n++)
									if (
										((e = k.getFile(n)),
										!l.supportedTypes || h.isTypeSupported(e, l.supportedTypes))
									) {
										var m = l.fileToElement(e);
										e = b.create(e, void 0, l.loaderType);
										m &&
											(e[g](l.uploadUrl, l.additionalRequestParameters),
											CKEDITOR.fileTools.markElement(m, c, e.id),
											("loadAndUpload" != g && "upload" != g) ||
												l.skipNotifications ||
												CKEDITOR.fileTools.bindNotifications(a, e),
				...
				CKEDITOR.tools.extend(f, {
						...
						k.on("update", function(b) {
							...
							if (d.wrapper && d.wrapper.getParent()) {
								...
								if (
									...
									(e = "cke_upload_" + k.status),
										d.wrapper &&
											e != g &&
											(g && d.wrapper.removeClass(g),
											d.wrapper.addClass(e),
											(g = e)),
					...
					replaceWith: function(d, c) {
						if ("" === d.trim()) a.widgets.del(this);
							...
							b
								? (g.moveToBookmark(e), g.select())
								: a.getSelection().selectBookmarks(h);
						}
					},
					_getLoader: function() {
			...
			bindNotifications: function(a, c) {
				function f() {
					...
					if (!h || h.isFinished())
						(h = a._.uploadWidgetNotificaionAggregator = new CKEDITOR.plugins.notificationAggregator(
							a,
				...
				c.on("update", function() {
					!b &&
						c.uploadTotal &&
						(f(), (b = h.createTask({ weight: c.uploadTotal })));
	...
	(function() {
		...
		function n(c) {
			...
			e += 1;
			return (
				"image-" + CKEDITOR.tools.array.map(a, l).join("") + "-" + e + "." + c
			);
		}
		var e = 0;
		CKEDITOR.plugins.add("uploadimage", {
			...
			init: function(c) {
				if (this.isSupportedEnvironment()) {
					...
					e &&
						(a.addUploadWidget(c, "uploadimage", {
							...
							onUploaded: function(b) {
								...
								this.replaceWith(
									'\x3cimg src\x3d"' +
										b.url +
										'" width\x3d"' +
										(b.responseData.width || a.naturalWidth) +
										'" height\x3d"' +
										(b.responseData.height || a.naturalHeight) +
										'"\x3e'
								);
							}
						}),
						c.on("paste", function(b) {
							if (b.data.dataValue.match(/<img[\s\S]+data:/i)) {
								...
								for (k = 0; k < m.count(); k++) {
									...
									h &&
										l &&
										!g.data("cke-upload-id") &&
										!g.isReadOnly(1) &&
										((h =
											((h = d.match(/image\/([a-z]+?);/i)) && h[1]) || "jpg"),
										(d = c.uploadRepository.create(d, n(h))),
										d.upload(e),
										a.markElement(g, "uploadimage", d.id),
										a.bindNotifications(c, d));
								}
