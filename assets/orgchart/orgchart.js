...
(function($) {
	...
	$.fn.copyAttributes = function(elem) {
		...
		$.each($(elem).prop("attributes"), function() {
			if (this.name != "class") $this.attr(this.name, this.value);
		});
		return $this;
	};
	...
	function buildNode($node, $appendTo, level, opts) {
		...
		if ($childNodesCount > 0) {
			...
			$nodeCell.append(
				'<div><img class="cover" src="../assets/orgchart/orgchart.minus.png"/></div>'
			);
		}
			...
			$nodeDiv
				...
				.click(function() {
					...
					if ($tr.hasClass("contracted")) {
						...
						$(this).attr("src", "../assets/orgchart/orgchart.minus.png");
						...
						$(this).attr("src", "../assets/orgchart/orgchart.plus.png");
		...
		if ($node.attr("class") != undefined) {
			...
			$.each(classList, function(index, item) {
				if (item == "collapsed") {
					...
					$nodeRow.find("img.cover").attr("src", "../assets/orgchart/orgchart.plus.png");
				} else {
